...
from Code.Simple_Utility import *
...
def bucket_distribution_tag(df, tag, y_pred, tgt_col, bench_mark, split_num, methods):
    ...
    if bench_mark == '':
        ...
        y_pred_bench = y_pred.iloc[np.where(data_all_tag == bench_mark)]
    ...
    for i in tag_list:
        print(i)
        y_pred_tmp = y_pred.iloc[np.where(data_all_tag == i)]
...
def loop_report_subgroup(df, tag, y_pred, group_column, my_dict):
    ...
    my_pairs = [('a', 'aa'), ('a', 'bb'), ('a', 'cc'), ('b', 'cc')] 如果标签重复，可以用这种方式循环。替换掉my_dict
    '''
    ...
    # for key, value in my_pairs:
        ...
        temp3 = pd.concat([temp1,temp2.iloc[:,6]],axis=1)
        temp3['sub_group'] = key
...
def psi_ks_auc(df, tag, y_pred, tgt_col, bench_mark, split_num = 10):
    ...
    auc = AUC(y_pred, y)
    psi = 0
    result_table.loc[len(result_table)] = ['Total', 'Total', ks, auc, psi]
    '''get split rules'''
    ...
    if bench_mark in ['', 'last_row']:
        ...
        y_pred_bench = y_pred.iloc[np.where(df[tag] == bench_mark)]

    tag_list = sorted(list(set(df[tag])))
    for i in tag_list:
        print(i)
        y_pred_tmp = y_pred.iloc[np.where(df[tag] == i)]
        y_tmp = y.iloc[np.where(df[tag] == i)].values
        try:
            ks = KS(y_pred_tmp, y_tmp)
            auc = AUC(y_pred_tmp, y_tmp)
            psi = PSI(y_pred_tmp, y_pred_bench, sorted(cut_point)) if bench_mark else 0
            if bench_mark == 'last_row':
                y_pred_bench = y_pred_tmp
            # '''if bench mark exists'''
            # if bench_mark == '':
            #     print(f'bench_mark is Total')
            #     psi = PSI(y_pred_tmp, y_pred_bench, bucket=sorted(cut_point))
            # elif bench_mark == 'last_row':
            #     print(f'bench_mark is always last one')
            #     psi = PSI(y_pred_tmp, y_pred_bench, bucket=sorted(cut_point))
            #     y_pred_bench = y_pred_tmp
            # else:
            #     print(f'bench_mark is {bench_mark}')
            #     psi = PSI(y_pred_tmp, y_pred_bench, bucket=sorted(cut_point))
